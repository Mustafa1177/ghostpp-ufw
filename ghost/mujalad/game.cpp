...
void CGame::EventPlayerJoined(CPotentialPlayer* potential, CIncomingJoinPlayer* joinPlayer)
{
	if(!m_Map->GetMapMatchMakingCategory().empty())
	{
		...
		m_PairedDPSChecksNew.push_back(PairedDPSCheckNew("%SomeRandomName", m_GHost->m_DB->ThreadedDotAPlayerSummaryCheckNew(string(), StatsUser, m_ScoreMinGames, GameState)));
